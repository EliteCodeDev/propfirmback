  ...
  async onModuleInit() {
    ...
    if (superAdmin && wildcard) {
      const exists = await this.rpRepo.findOne({
        where: { roleID: superAdmin.roleID, permissionID: wildcard.permissionID },
      });
      if (!exists) {
        const rp = this.rpRepo.create({ roleID: superAdmin.roleID, permissionID: wildcard.permissionID });
